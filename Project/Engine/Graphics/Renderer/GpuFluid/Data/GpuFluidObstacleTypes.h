...
enum class GpuFluidObstacleShape : uint32_t
{
	Sphere = 0,
	Box,
};

/// Physics ColliderからRendererへ渡す依存の薄いObstacle Sourceデータ。
struct GpuFluidObstacleSource
{
	GpuFluidObstacleShape shape = GpuFluidObstacleShape::Sphere;
	...
	[[nodiscard]] bool IsValid() const
	{
		...
		if (shape == GpuFluidObstacleShape::Sphere)
		{
			return radius > 0.0f;
		}
...
/// StructuredBufferへ詰めるGPU側Obstacle要素。HLSL側と96-byteで一致させる。
struct alignas(16) GpuFluidObstacleGpuData
{
...
static_assert(sizeof(GpuFluidObstacleGpuData) == 96);

/// Obstacle Raster用のWorld Domain情報。HLSL側と48-byteで一致させる。
struct alignas(16) GpuFluidObstacleRasterConstants
{
	float originX = 0.0f;
	float originY = 0.0f;
	float originZ = 0.0f;
...
static_assert(sizeof(GpuFluidObstacleRasterConstants) == 48);

inline bool BuildGpuFluidObstacleGpuData(
	const GpuFluidObstacleSource& source,
	GpuFluidObstacleGpuData& outData)
{
	if (!source.IsValid())
	{
		return false;
...
inline GpuFluidObstacleRasterConstants BuildGpuFluidObstacleRasterConstants(
	...
	uint32_t obstacleCount)
{
	...
	GpuFluidObstacleRasterConstants constants{};
