...
namespace Ken4lowEngine
{
	namespace
	{
		...
		bool SamePosition(const Vector3& lhs, const Vector3& rhs)
		{
			return NearlyEqual(lhs.x, rhs.x) && NearlyEqual(lhs.y, rhs.y) && NearlyEqual(lhs.z, rhs.z);
		}

		uint32_t SanitizeResolution(uint32_t resolution)
		{
	...
	void ReflectionProbeManager::Initialize(DirectXCommon* dxCommon)
	{
		Finalize();
		dxCommon_ = dxCommon;
		captureSerial_ = 0;
	...
	void ReflectionProbeManager::Finalize()
	{
	...
	ReflectionProbeDesc ReflectionProbeManager::SanitizeDesc(const ReflectionProbeDesc& desc) const
	{
		ReflectionProbeDesc sanitized = desc;
		sanitized.influenceRadius = std::max(desc.influenceRadius, kMinimumProbeRadius);
		sanitized.nearClip = std::max(desc.nearClip, kMinimumNearClip);
		sanitized.farClip = std::max(desc.farClip, sanitized.nearClip + kMinimumFarGap);
		sanitized.resolution = SanitizeResolution(desc.resolution);
		return sanitized;
	}

	void ReflectionProbeManager::RegisterOrUpdateProbe(const void* owner, const ReflectionProbeDesc& desc, bool forceDirty)
	{
		if (!owner) return;
		...
		const bool captureInputChanged =
			!SamePosition(probe->desc.position, sanitized.position) ||
			!NearlyEqual(probe->desc.nearClip, sanitized.nearClip) ||
			!NearlyEqual(probe->desc.farClip, sanitized.farClip) ||
	...
	void ReflectionProbeManager::UnregisterProbe(const void* owner)
	{
		ProbeRuntime* probe = FindProbe(owner);
		if (!probe) return;
		probe->owner = nullptr;
	...
	void ReflectionProbeManager::RequestCapture(const void* owner)
	{
	...
	ReflectionProbeManager::ProbeRuntime* ReflectionProbeManager::FindProbe(const void* owner)
	{
	...
	const ReflectionProbeManager::ProbeRuntime* ReflectionProbeManager::FindProbe(const void* owner) const
	{
		if (!owner) return nullptr;
		const auto found = std::find_if(probes_.begin(), probes_.end(),
			[owner](const ProbeRuntime& probe) { return probe.owner == owner; });
	...
	ReflectionProbeManager::ProbeRuntime* ReflectionProbeManager::FindCaptureCandidate()
	{
		for (ProbeRuntime& probe : probes_)
		{
	...
	bool ReflectionProbeManager::CapturePending(const std::function<void()>& drawStaticScene)
	{
	...
	bool ReflectionProbeManager::EnsureTarget(ProbeRuntime& probe)
	{
	...
	std::unique_ptr<ReflectionProbeManager::ProbeTarget> ReflectionProbeManager::CreateTarget(uint32_t resolution)
	{
		if (!dxCommon_ || !dxCommon_->GetDevice()) return nullptr;
		auto target = std::make_unique<ProbeTarget>();
		target->resolution = SanitizeResolution(resolution);

		D3D12_RESOURCE_DESC colorDesc{};
		colorDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
		colorDesc.Width = target->resolution;
		colorDesc.Height = target->resolution;
		colorDesc.DepthOrArraySize = static_cast<UINT16>(kCubeFaceCount);
	...
	void ReflectionProbeManager::TransitionTarget(ProbeTarget& target, D3D12_RESOURCE_STATES nextState)
	{
	...
	void ReflectionProbeManager::BeginFace(ProbeTarget& target, uint32_t faceIndex)
	{
		if (faceIndex >= kCubeFaceCount || !target.color || !target.depth) return;
	...
	void ReflectionProbeManager::EndCapture(ProbeTarget& target)
	{
		TransitionTarget(target, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
	}

	void ReflectionProbeManager::ReleaseTarget(ProbeTarget& target)
	{
		RTVManager* rtvManager = RTVManager::GetInstance();
	...
	void ReflectionProbeManager::SyncCaptureSkyBox()
	{
	...
	bool ReflectionProbeManager::CaptureProbe(ProbeRuntime& probe, const std::function<void()>& drawStaticScene)
	{
		if (!EnsureTarget(probe) || !probe.target) return false;
		SyncCaptureSkyBox();

		const std::array<Vector3, kCubeFaceCount> directions = {
			Vector3{ 1.0f, 0.0f, 0.0f }, Vector3{ -1.0f, 0.0f, 0.0f },
			Vector3{ 0.0f, 1.0f, 0.0f }, Vector3{ 0.0f, -1.0f, 0.0f },
			Vector3{ 0.0f, 0.0f, 1.0f }, Vector3{ 0.0f, 0.0f, -1.0f }
		};
		const std::array<Vector3, kCubeFaceCount> upVectors = {
			Vector3{ 0.0f, 1.0f, 0.0f }, Vector3{ 0.0f, 1.0f, 0.0f },
			Vector3{ 0.0f, 0.0f, -1.0f }, Vector3{ 0.0f, 0.0f, 1.0f },
			Vector3{ 0.0f, 1.0f, 0.0f }, Vector3{ 0.0f, 1.0f, 0.0f }
		};
	...
	D3D12_GPU_DESCRIPTOR_HANDLE ReflectionProbeManager::ResolveReflectionHandle(const Vector3& worldPosition) const
	{
		if (isCapturing_)
		{
			return EnvironmentMapManager::GetInstance()->GetEnvironmentMapHandle();
		}
		...
		return EnvironmentMapManager::GetInstance()->GetEnvironmentMapHandle(); // Probe圏外は従来のSkyBox Environmentを安全な最終Fallbackにする。
	}

	ReflectionProbeDiagnostics ReflectionProbeManager::GetDiagnostics(const void* owner) const
	{
	...
	uint32_t ReflectionProbeManager::GetCapturedProbeCount() const
	{
		uint32_t count = 0;
		for (const ProbeRuntime& probe : probes_)
		{
