...
{
	namespace ReflectionProbeComponentDetail
	{
		inline const char* UpdateModeToString(ReflectionProbeUpdateMode updateMode)
		{
			switch (updateMode)
			{
		...
		inline ReflectionProbeUpdateMode UpdateModeFromString(const std::string& value)
		{
		...
		inline uint32_t SanitizeProbeResolution(uint32_t resolution)
		{
	...
	inline void ReflectionProbeComponent::Initialize()
	{
	...
	inline void ReflectionProbeComponent::Update(float deltaTime)
	{
		SceneComponent::Update(deltaTime);
		SyncToManager();
	}

	inline void ReflectionProbeComponent::UpdateEditor(float deltaTime)
	{
		SceneComponent::UpdateEditor(deltaTime);
		SyncToManager(); // Editor Gizmoで移動したProbeもPlayせず次Captureへ反映する。
	}

	inline void ReflectionProbeComponent::Draw()
	{
	...
	inline void ReflectionProbeComponent::DrawImGui()
	{
	...
	inline void ReflectionProbeComponent::Finalize()
	{
		ReflectionProbeManager::GetInstance()->UnregisterProbe(this);
	}

	inline void ReflectionProbeComponent::ToJson(nlohmann::json& outJson) const
	{
		...
		outJson["UpdateMode"] = ReflectionProbeComponentDetail::UpdateModeToString(updateMode_);
		outJson["DebugBoundsVisible"] = debugBoundsVisible_;
	}

	inline void ReflectionProbeComponent::FromJson(const nlohmann::json& inJson)
	{
		...
		if (const auto it = inJson.find("Resolution"); it != inJson.end() && it->is_number_integer()) resolution_ = static_cast<uint32_t>(std::max(0, it->get<int>()));
		if (const auto it = inJson.find("UpdateMode"); it != inJson.end() && it->is_string()) updateMode_ = ReflectionProbeComponentDetail::UpdateModeFromString(it->get<std::string>());
		if (const auto it = inJson.find("DebugBoundsVisible"); it != inJson.end() && it->is_boolean()) debugBoundsVisible_ = it->get<bool>();
		Sanitize();
	}

	inline void ReflectionProbeComponent::SyncToManager(bool forceDirty)
	{
		RefreshWorldTransform();
		ReflectionProbeManager::GetInstance()->RegisterOrUpdateProbe(this, BuildDesc(), forceDirty);
	}

	inline void ReflectionProbeComponent::RequestCapture()
	{
		SyncToManager();
		ReflectionProbeManager::GetInstance()->RequestCapture(this);
	}

	inline void ReflectionProbeComponent::SetInfluenceRadius(float radius)
	{
		influenceRadius_ = std::max(radius, 0.1f);
	}

	inline void ReflectionProbeComponent::SetNearClip(float nearClip)
	{
		nearClip_ = std::max(nearClip, 0.01f);
		farClip_ = std::max(farClip_, nearClip_ + 0.1f);
	}

	inline void ReflectionProbeComponent::SetFarClip(float farClip)
	{
		farClip_ = std::max(farClip, nearClip_ + 0.1f);
	}

	inline void ReflectionProbeComponent::SetResolution(uint32_t resolution)
	{
		resolution_ = ReflectionProbeComponentDetail::SanitizeProbeResolution(resolution);
	}

	inline ReflectionProbeDesc ReflectionProbeComponent::BuildDesc() const
	{
	...
	inline void ReflectionProbeComponent::Sanitize()
	{
		influenceRadius_ = std::max(influenceRadius_, 0.1f);
		nearClip_ = std::max(nearClip_, 0.01f);
		farClip_ = std::max(farClip_, nearClip_ + 0.1f);
		resolution_ = ReflectionProbeComponentDetail::SanitizeProbeResolution(resolution_);
	}
} // namespace Ken4lowEngine
