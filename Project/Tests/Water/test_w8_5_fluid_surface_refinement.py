...
class W85FluidSurfaceRefinementTests(unittest.TestCase):
    @classmethod
    ...
    def test_reset_keeps_the_3d_grid_contract_but_adds_deterministic_jitter(self) -> None:
        self.assertIn("particle.position = gSph.spawnOrigin + float3(x, y, z) * gSph.spawnSpacing", self.reset_cs)
        self.assertIn("W85Hash", self.reset_cs)
        self.assertIn("W85Hash01", self.reset_cs)
        self.assertIn("gSph.spawnSpacing * 0.08f", self.reset_cs)
        self.assertIn("particle.predictedPosition = particle.position", self.reset_cs)
