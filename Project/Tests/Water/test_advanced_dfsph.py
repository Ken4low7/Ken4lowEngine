...
class AdvancedDfSphTests(unittest.TestCase):
    @classmethod
    ...
    def test_particle_contract_and_simulation_constants(self) -> None:
    ...
    def test_persistent_dfsph_state_is_bound_by_manager(self) -> None:
        ...
        self.assertIn("PersistentDfSphState", self.manager_cpp)
    ...
    def test_manifest_registers_projection_and_metric_passes(self) -> None:
        for marker in (
            "DfSphFactor",
            "DfSphDensityPrepare",
            "DfSphDensityApply",
    ...
    def test_dfsph_uses_spatial_hash_and_iterative_projection(self) -> None:
    ...
    def test_wc_sph_remains_a_runtime_fallback(self) -> None:
        ...
        self.assertIn("WCSPH / Tait EOS", self.panel)
    ...
    def test_warm_start_only_applies_on_first_projection_iteration(self) -> None:
        ...
        self.assertIn('ImGui::Checkbox("前フレームの解を利用"', self.panel)

    def test_gpu_metrics_reduce_without_waiting(self) -> None:
    ...
    def test_diagnostics_panel_exposes_convergence_metrics_and_stress_presets(self) -> None:
        self.assertIn('ImGui::Begin("SPH詳細設定 / DFSPH"', self.panel)
        self.assertIn("最大密度誤差", self.panel)
        self.assertIn("最大発散誤差", self.panel)
        self.assertIn("最大粒子速度", self.panel)
