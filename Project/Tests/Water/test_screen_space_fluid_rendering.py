...
class ScreenSpaceFluidRenderingTests(unittest.TestCase):
    @classmethod
    ...
    def test_uses_existing_sph_particle_buffer_without_copying_particles(self) -> None:
    ...
    def test_runs_after_scene_draw_and_before_post_effect_transition(self) -> None:
    ...
    def test_render_targets_define_matching_optimized_clear_values(self) -> None:
    ...
    def test_visual_tuning_window_exposes_surface_controls_and_stats(self) -> None:
        self.assertIn('ImGui::Begin("スクリーンスペース流体")', self.post_cpp)
        self.assertIn('"粒子描画半径"', self.post_cpp)
        self.assertIn('"深度ぼかし境界"', self.post_cpp)
        self.assertIn('"屈折の強さ"', self.post_cpp)
        self.assertIn('"水向け描画設定を適用"', self.post_cpp)
        self.assertIn('"描画状況"', self.post_cpp)
        self.assertIn("particleDepthDrawCount", self.post_cpp)
        self.assertIn("compositeDrawCount", self.post_cpp)  # 日本語化後も描画統計の表示を維持する。
