...
class SphParticleBufferTests(unittest.TestCase):
    @classmethod
    ...
    def test_cpu_particle_layout_is_48_bytes(self) -> None:
    ...
    def test_manager_keeps_particle_capacity_contract(self) -> None:
    ...
    def test_diagnostics_exposes_particle_storage_contract(self) -> None:
        self.assertIn('SPHシミュレーション', self.diagnostics)
        self.assertIn('粒子Buffer: %s', self.diagnostics)
        self.assertIn('使用中 / 最大: %u / %u', self.diagnostics)
        self.assertIn('粒子Stride: %u bytes', self.diagnostics)
        self.assertIn('SRV: %u | Compute SRV: %u | UAV: %u', self.diagnostics)

    def test_build_props_registers_sph_sources(self) -> None:
        for path in (
            ...
            self.assertIn(path, self.build_props)  # Build設定から主要SPHソースが落ちていないことを確認する。
