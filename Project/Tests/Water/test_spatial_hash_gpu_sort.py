...
class SpatialHashGpuSortTests(unittest.TestCase):
    @classmethod
    ...
    def test_cpp_and_hlsl_constant_layout_matches_extended_runtime(self) -> None:
    ...
    def test_root_signature_preserves_spatial_hash_bindings(self) -> None:
    ...
    def test_neighbor_forces_use_27_cells_and_do_not_scan_every_particle(self) -> None:
    ...
    def test_manifest_preserves_spatial_hash_passes(self) -> None:
        for marker in (
            "SpatialBuildKeys",
            "SpatialBitonicSort",
    ...
    def test_diagnostics_exposes_grid_sort_and_dispatch_status(self) -> None:
        for marker in (
            "空間ハッシュ / GPUソート",
            "空間ハッシュ: %s",
            "Grid: %u x %u x %u | Cell数: %u",
            "Bitonic Sort数",
            "Hash構築",
            "Sort Dispatch",
            "Cell範囲Dispatch",
        ):
            self.assertIn(marker, self.diagnostics)  # 工程番号を外した後も空間ハッシュ診断が残ることを確認する。
