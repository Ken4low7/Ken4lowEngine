...
def test_w9_supports_four_runtime_collider_shapes():
...
def test_w9_uses_frame_resource_readback_without_gpu_waits():
    source = read(INTERACTION)
...
def test_w9_delayed_reactions_use_actor_handles_not_persistent_actor_pointers():
    source = read(INTERACTION)
...
def test_w9_gpu_reaction_is_atomic_fixed_point_force_and_torque():
    shader = read(SHADER)
...
def test_w9_particle_collision_updates_position_velocity_and_prediction():
    shader = read(SHADER)
...
def test_w9_readback_applies_linear_and_angular_impulses_to_dynamic_bodies():
    source = read(INTERACTION)
...
def test_w9_is_connected_once_at_main_scene_bridge_and_not_editor_edit_mode():
...
def test_w9_resources_finalize_before_particle_buffer_release():
...
def test_w9_diagnostics_and_stress_limits_are_exposed():
    source = read(INTERACTION)
    ...
    assert "Collider Proxies" in diagnostics
    assert "Dynamic Bodies" in diagnostics
    assert "Async Readbacks" in diagnostics
    assert "Applied Rigidbody Reactions" in diagnostics
    assert "Last Linear Impulse" in diagnostics
    assert "Last Angular Impulse" in diagnostics
    assert "GpuSphRigidbodyInteractionDiagnosticsPanel::GetInstance()->Draw();" in overlay
